...
export function createParagraphNode(children?: DataNode[], title?: string, memo?: string): ParagraphNode {
  children = children ?? [];
...
export function insertNewNodeAfterTextNode(node: TextNode, newNode: DataNode, copyInformations?: boolean): ParagraphNode {
...
export function insertNewNodeBeforeTextNode(node: TextNode, newNode: DataNode, copyInformations?: boolean): ParagraphNode {
  const thisNode: ConvertionNode = (node as DataNode);
...
export function insertNewNodeToTextNode(node: TextNode, newNode: DataNode, offset: number): ParagraphNode {
  const thisNode: ConvertionNode = (node as DataNode);
...
export function insertNewNodeToParagraphNode(node: ParagraphNode, newNode: DataNode, offset: number): ParagraphNode {
  node.children = [
...
export function canonicalizeNodes(node: DataNode): DataNode | null {
  ...
  node.children = node.children.map(it => canonicalizeNodes(it)).filter(it => it !== null) as DataNode[];
  return node;
}
